import { AuditRequest, AuditReport, AuditFinding, AgentTraceEntry } from "../src/types";
...
export async function runAudit(
  ...
  // Step 0: metadata check
  onProgress({ type: "step", id: "metadata-check", status: "running", title: "Checking dataset metadata" });
  const metaFindings = metadataCheck(request);
  onProgress({
    type: "step", id: "metadata-check", status: "done", title: "Checking dataset metadata",
    detail: metaFindings.length > 0 ? `Found ${metaFindings.length} metadata issue(s)` : "Metadata looks clean",
  });
  ...
  let agentTrace: AgentTraceEntry[] = [];
  try {
    const reviewResult = await reviewAgent(request, phase1Findings);
    additionalFindings = reviewResult.findings;
    agentTrace = reviewResult.trace;
  } catch (error) {
  ...
  const hasLowConfidence = allFindings.some((f) => f.confidence === "low");
  const hasEscalations = allFindings.some((f) => f.escalate_reason);
  let disclaimer = "";
  if (hasLowConfidence || hasEscalations) {
    disclaimer = "Note: Some findings have low confidence or were escalated for human review. " +
      "This audit is generated by an AI system and should be validated by a domain expert before making pipeline changes.";
  }

  const narrativeWithDisclaimer = disclaimer
    ? `${full_report}\n\n---\n${disclaimer}`
    : full_report;
  ...
  return {
    ...
    executive_summary,
    narrative_report: narrativeWithDisclaimer,
